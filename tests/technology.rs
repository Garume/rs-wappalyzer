...
use rs_wappalyzer::technology::{DomComplex, DomElement, TechnologyJson};
...
fn deserialize_example_technology() {
    ...
    assert_eq!(
        ...
        Some(&"(?:Example|Another Example)".to_string())
    );
    assert_eq!(
    ...
    assert_eq!(technology.url, Some("example\\.com".to_string()));
    assert_eq!(technology.xhr, Some("example\\.com".to_string()));
