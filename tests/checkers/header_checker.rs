...
use rs_wappalyzer::checkers::checker::Checker;
use rs_wappalyzer::checkers::header_checker::HeaderChecker;
use rs_wappalyzer::technology::Technology;
...
fn checker_works_with_regex() {
    ...
    technology
        .headers
        .insert("Content-Type".to_string(), WappalyzerRegex::new("text/.*"));
    let result = checker.check(&webpage, &technology);
    assert_eq!(result.is_some(), true);
    assert_eq!(result.unwrap().name, "Example".to_string());
}
...
fn checker_works_with_regex_and_version() {
    ...
    technology.headers.insert(
        "server".to_string(),
        WappalyzerRegex::new("server/?([\\d.]+)?\\;version:\\1"),
    );
...
fn checker_works_with_regex_fail() {
    ...
    technology
        .headers
        .insert("Content-Type".to_string(), WappalyzerRegex::new("text/css"));
    let result = checker.check(&webpage, &technology);
